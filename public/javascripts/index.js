let host = 'http://10.176.168.58:3000'
function init() {
  let domArr = Array.prototype.slice.call(document.querySelectorAll('p'))
  console.log(domArr)
  domArr.map(item => {
...
// axios会将ctx。body赋值给data，因此将data重新包装为一个blob
function onClick(e) {
  let text = e.target.innerText
  console.log(text)
  axios.get(`${host}/down/${text}`).then(async res => {
    console.log(res)
    // let blob = await res.blob()
    let objectURL = window.URL.createObjectURL(new Blob([res.data]))
    // console.log(blob)
    // objectURL = window.URL.createObjectURL(blob)
    download(objectURL, text)
  })
}
...
//   let text = e.target.innerText
//   console.log(text)
//   fetch(`${host}/down/${text}`).then(async res => {
//     console.log(res)
//     let blob = await res.blob()
//     // console.log(blob)
//     objectURL = window.URL.createObjectURL(blob)
...
function download(blobUrl, text) {
